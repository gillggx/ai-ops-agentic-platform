...
// Always route through the main server's proxy path (/simulator-api).
// The main server proxies to the ontology_simulator backend on port 8099.
// Evaluated lazily at call-time to avoid SSG/hydration mismatch.
function getUrls() {
  if (typeof window === "undefined") {
    return { ws: "ws://localhost:8000/simulator-api/ws", api: "http://localhost:8000/simulator-api/api/v1" };
  }
  const wsp = window.location.protocol === "https:" ? "wss" : "ws";
  return {
    ws:  `${wsp}://${window.location.host}/simulator-api/ws`,
