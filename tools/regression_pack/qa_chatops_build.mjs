...
await page.screenshot({ path: `${OUT}/qa_co_idle.png` });

// send build request (user's case)
const input = page.locator('textarea, input[placeholder*="輸入"]').last();
await input.fill("查 STEP_007 過去 7 天所有機台的 xbar_chart 趨勢，合併成一張圖：x 軸按時間排序，用顏色區分機台");
...
await page.screenshot({ path: `${OUT}/qa_co_plan.png` });
...
await page.screenshot({ path: `${OUT}/qa_co_building.png` });
...
await page.screenshot({ path: `${OUT}/qa_co_result.png`, fullPage: false });
...
if (result === "done") {
  ...
  await page.screenshot({ path: `${OUT}/qa_co_done_full.png` });
}
...
await page.screenshot({ path: `${OUT}/qa_co_after.png` });
