...
#
# POC skill-library branch — 3 active 服務（simulator removed）：
#   1. 系統依賴（Python 3.11, Node.js 20, Java 17, PostgreSQL, Redis, Nginx, Certbot）
#   2. 建立 /opt/aiops 目錄
#   3. Python virtualenv (sidecar)
#   4. PostgreSQL DB + user
#   5. systemd 服務（aiops-app + aiops-java-api + aiops-python-sidecar）
#   6. Nginx + SSL + UFW
#
...
# MongoDB removed in POC (was simulator-only).
...
for env_file in "$JAVA_ENV" "$SIDECAR_ENV"; do
  if [ ! -f "$env_file" ]; then
...
# 3 active 服務（POC: simulator removed）
for unit in aiops-app aiops-java-api aiops-python-sidecar; do
...
ufw --force enable
echo "    ✅  UFW: SSH + HTTP + HTTPS 開放；8000/8002/8050 不對外"
...
echo "      bash deploy/update.sh        # aiops-app"
echo ""
echo "  🔍  狀態檢查："
echo "      systemctl status aiops-app aiops-java-api aiops-python-sidecar"
echo ""
echo "  📝  Log："
