...
# ── Python interpreters (use repo venvs if present) ──────────────────────────
...
# Fall back to system python if venvs don't exist
...
nohup "$BACKEND_UVICORN" main:app --host 0.0.0.0 --port 8000 --reload > "$LOG_FAST" 2>&1 &
FAST_PID=$!
