...
@router.delete("/memory/source/{source}", summary="清除指定類型的記憶")
async def delete_memory_by_source(
    ...
    svc = AgentMemoryService(db)
    count = await svc.delete_by_source(current_user.id, source)
    return {"status": "success", "source": source, "deleted_count": count}
