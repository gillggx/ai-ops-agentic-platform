...
class MCPDefinitionUpdate(BaseModel):
    ...
    prefer_over_system: Optional[bool] = None
    api_config: Optional[Dict[str, Any]] = None
    input_schema: Optional[Dict[str, Any]] = None
...
class MCPAgentBuildRequest(BaseModel):
    """Agent-initiated full MCP build: sample-fetch → try-run → create → generate."""
    name: str = Field(..., min_length=1, max_length=200)
    description: str = Field(default="")
    system_mcp_id: int = Field(..., description="System MCP to build on top of")
    processing_intent: str = Field(..., min_length=1)
    sample_params: Optional[Dict[str, Any]] = Field(default=None, description="Params for sample-fetch; omit to use defaults")


class MCPAgentBuildResponse(BaseModel):
    """Result of an agent-initiated MCP build."""
    success: bool
