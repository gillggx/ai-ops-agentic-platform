...
class MCPDefinitionModel(Base):
    ...
    # When True: the system_mcp_id entry is hidden from the agent's mcp_catalog so the
    # agent always calls this custom MCP instead of the underlying system MCP directly.
    prefer_over_system: Mapped[bool] = mapped_column(Boolean, nullable=False, default=False, server_default="0")
