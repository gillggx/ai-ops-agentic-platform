...
【AIOps Generic Tools — 優先使用，大幅減少手工計算】
`tools` 物件已預注入，包含 100+ 個分析與繪圖函式，直接呼叫即可（無需 import）。

所有 tools 函式回傳格式：{"status": "success"|"error", "summary": str, "payload": {...}}
- 使用前必須判斷 result["status"] == "success" 才可取用資料
- 繪圖函式的 payload 格式：{"plotly": <plotly_dict>}
- 取得繪圖 JSON 字串：json.dumps(result["payload"]["plotly"])

⭐ 半導體製程最常用函式：

【統計 / 製程品質】
tools.calc_statistics(data, column="value")
  → {"mean", "std", "median", "skewness", "kurtosis", ...}
tools.find_outliers(data, column="value", method="sigma", threshold=3.0)
  → 異常資料列清單
tools.capability_analysis(data, column="value", usl=..., lsl=...)
  → {"Cp", "Cpk", "Pp", "Ppk", "sigma_level", ...}
tools.western_electric_rules(data, column="value", ucl=..., lcl=...)
  → Western Electric 違規清單
tools.nelson_rules(data, column="value", ucl=..., lcl=...)
  → Nelson 違規清單
tools.control_limits_calculator(data, column="value")
  → {"ucl", "lcl", "mean"}
tools.process_sigma(data, column="value", usl=..., lsl=...)
  → sigma level 數值

【時序分析】
tools.ewma_smoothing(data, column="value", alpha=0.3)
  → EWMA 平滑後的資料列清單
tools.cusum_detection(data, column="value")
  → CUSUM 累積和分析結果
tools.detect_step_change(data, column="value")
  → 製程偏移點清單
tools.rolling_statistics(data, column="value", window=10)
  → 移動平均 / 移動標準差

【繪圖 — 直接產生 Plotly JSON 字串加入 charts 陣列】
tools.plot_xbar_r(data, column="value", subgroup_size=5)   → Xbar-R 管制圖
tools.plot_imr(data, column="value")                        → I-MR 管制圖
tools.plot_cusum(data, column="value")                      → CUSUM 圖
tools.plot_ewma_chart(data, column="value")                 → EWMA 圖
tools.plot_capability_hist(data, column="value", usl=..., lsl=...) → 製程能力直方圖
tools.plot_pareto(data, category_col="defect_type", value_col="count") → Pareto 圖
tools.plot_spc_dashboard(data, column="value")              → SPC Dashboard
tools.plot_line(data, x="ts", y="value")                    → 折線圖
tools.plot_bar(data, x="category", y="value")               → 長條圖
tools.plot_scatter(data, x="param_a", y="param_b")          → 散點圖

⭐ 繪圖工具正確使用範例：
  charts = []
  r = tools.plot_xbar_r(raw_data, column="measured_value", subgroup_size=5)
  if r["status"] == "success":
      charts.append(json.dumps(r["payload"]["plotly"]))
  ui_render = {"type": "trend_chart", "charts": charts, "chart_data": charts[0] if charts else None}

⚠️ tools 函式本身已處理所有 import（plotly、numpy 等），無需在腳本中重複 import。
...
process() 函式的主要邏輯必須包在 try/except 中：
  try:
      # ① 開頭先取得可用欄位（防止 KeyError 時能提示哪些欄位存在）
      _sample = raw_data[0] if isinstance(raw_data, list) and raw_data else raw_data if isinstance(raw_data, dict) else {}
      _available_keys = list(_sample.keys()) if isinstance(_sample, dict) else []
      # ② 主要計算邏輯（使用 _available_keys 確認欄位名再 access）
  except Exception as e:
      _sample = raw_data[0] if isinstance(raw_data, list) and raw_data else raw_data if isinstance(raw_data, dict) else {}
      _keys = list(_sample.keys()) if isinstance(_sample, dict) else []
      return {"output_schema": {"fields": []}, "dataset": [], "ui_render": {"type": "table", "charts": [], "chart_data": None},
              "_error": f"執行異常：{e}（資料可用欄位：{_keys}）"}
不得使用 raise，遇到錯誤一律回傳 _error 欄位。
不得有空 if 區塊（if: 後面必須有程式碼）。
datetime 可直接 from datetime import datetime, timedelta, timezone 使用。

【raw_data 結構正規化 — process() 開頭必須做】
許多 API 回傳的是 wrapper 物件，實際資料在 "data" 或 "records" 子欄位。
process() 開頭必須先解包：
  # ── 標準正規化：支援 list / wrapper-dict / bare-dict ─────────────────
  if isinstance(raw_data, list):
      rows = raw_data
  elif isinstance(raw_data, dict):
      # 常見 wrapper：{"data": [...], "total": N, ...} 或 {"records": [...]}
      if "data" in raw_data and isinstance(raw_data["data"], list):
          rows = raw_data["data"]
      elif "records" in raw_data and isinstance(raw_data["records"], list):
          rows = raw_data["records"]
      else:
          rows = [raw_data]
  else:
      rows = []
  if not rows:
      return {"output_schema":{"fields":[]},"dataset":[],"ui_render":{"type":"table","charts":[],"chart_data":None}}
  # ── 可用欄位（供 except 除錯用）──────────────────────────────────────
  _available_keys = list(rows[0].keys()) if rows else []
