...
Extended in AIOps Platform:
  - Accepts optional EventContext for structured audit trail
  - Writes ExecutionLog record for every run (started_at, finished_at, status, llm_readable_data)
  - Prefers active ScriptVersion from Script Registry over inline last_diagnosis_result
"""
...
class SkillExecuteService:
    ...
    def __init__(
        ...
        script_repo: Optional[ScriptVersionRepository] = None,
        log_repo: Optional[ExecutionLogRepository] = None,
    ) -> None:
    ...
    async def execute(
        ...
        If script_repo and log_repo are provided:
        - Prefers active ScriptVersion from Script Registry over inline last_diagnosis_result
        - Writes ExecutionLog record (started_at → finished_at) for audit trail
        """
        # Create execution log entry (if log_repo available)
        t0 = time.monotonic()
        log = None
        if self._log_repo:
            log = await self._log_repo.create(
                ...
                event_context=event_context.model_dump() if event_context else None,
            )
        ...
        # Priority: active ScriptVersion from Script Registry > inline last_diagnosis_result
        active_script = None
        script_version_id: Optional[int] = None
        if self._script_repo:
            active_script = await self._script_repo.get_active(skill_id)
        if active_script:
            diagnose_code = active_script.code
        ...
        if not diagnose_code:
            ...
            if log and self._log_repo:
                await self._log_repo.finish(
                    log, status="error",
                    error_message="缺少 diagnose code",
                    duration_ms=int((time.monotonic() - t0) * 1000),
                )
            return err_payload

        # Update log with resolved script_version_id
        if log and script_version_id and self._log_repo:
            log.script_version_id = script_version_id
        ...
        try:
            ...
            if log and self._log_repo:
                await self._log_repo.finish(
                    log, status="timeout" if isinstance(exc, TimeoutError) else "error",
