...
function _handleV13Event(ev) {
  ...
  switch (ev.type) {
    ...
    case 'context_load': {
      ...
      // Show first memory hit preview so user knows what was retrieved
      const hits = Array.isArray(ev.rag_hits) ? ev.rag_hits : [];
      const memPreview = hits.length > 0
        ? ` | 記憶命中：${(hits[0].content || '').slice(0, 60)}…`
        : '';
      _diagLogLine('📦', `CTX | RAG 記憶: ${ragCount} 條 | 歷史: ${turns} 輪${memPreview}`, '#60a5fa');
      break;
    }
    ...
    case 'memory_write': {
      ...
      if (mtype === 'trap') {
        ...
        _diagLogLine('💡',
          ...
          'learn');
      } else if (mtype === 'pattern' || ev.source === 'success_pattern') {
        // Extract tool chain from content if available
        const contentStr = ev.content || '';
        const chainMatch = contentStr.match(/工具鏈：([^|]+)/);
        const chain = chainMatch ? chainMatch[1].trim() : (ev.source || '?');
        _diagLogLine('💡',
          `[成功模式記憶] 工具鏈：${chain}${conflict}`,
          'learn');
      } else {
        _diagLogLine('💡',
          `[記憶寫入 · ${ev.source || '?'}] ${(ev.content || '').slice(0, 80)}${conflict}`,
          'learn');
      }
      break;
    }
    ...
    case 'reflection_running': {
      _diagLogLine('🔍', `Self-Critique 驗證中…（${ev.message || '數值來源交叉比對'}）`, '#34d399');
      break;
    }

    case 'reflection_pass': {
      _diagLogLine('✅', 'Self-Critique 通過 — 所有數值可追溯至工具回傳', '#34d399');
      break;
    }

    case 'reflection_amendment': {
      const issueCount = ev.issue_count || (ev.issues || []).length;
      _diagLogLine('⚠️', `Self-Critique 發現 ${issueCount} 處無來源數值，已修訂答案`, '#f97316');
      break;
    }
...
// Backend sends stages: 1, 2, 4, 5, 6 (stage 3 = tool loop, no stage_update)
// Dot mapping: backend 1→dot1, 2→dot2, 4→dot3+4, 5→dot5, 6→dot6
const _V16_STAGE_MAP = {
  1: { code: 'S1', name: 'Context Load',     label: '記憶檢索 + Soul 載入', color: '#60a5fa', dot: 1 },
  2: { code: 'S2', name: 'Planning',         label: '意圖規劃編排',          color: '#a78bfa', dot: 2 },
  4: { code: 'S4', name: 'Reasoning',        label: '工具執行 + 邏輯彙整',   color: '#fbbf24', dot: 4 },
  5: { code: 'S5', name: 'Self-Critique',    label: '數值來源驗證',          color: '#34d399', dot: 5 },
  6: { code: 'S6', name: 'Memory Learning',  label: '知識蒸餾與記憶寫入',   color: '#f472b6', dot: 6 },
};
...
function _v14UpdateStageBar(activeStage, status) {
  ...
  // Map backend stage number to dot number (backend skips 3; 4→dot4, 5→dot5, 6→dot6)
  const activeDot = (_V16_STAGE_MAP[activeStage] || {}).dot || activeStage;
  for (let i = 1; i <= 6; i++) {
...
async function _brainResetTrapMemories() {
  if (!confirm('清除所有 Trap 類型記憶（工具錯誤自動記錄）？')) return;
  try {
    const r = await fetch('/api/v1/agent/memory/source/trap', {
      method: 'DELETE',
      headers: { 'Authorization': `Bearer ${_token}` }
    });
    const d = await r.json();
    if (d.status === 'success') {
      alert(`已清除 ${d.deleted_count} 條 Trap 記憶`);
      _brainLoadMemories();
    }
  } catch(e) {
...
function _renderTraceContent(tab) {
  ...
  if (tab === 'reasoning') {
    ...
    if (_traceStages.length) {
      // Index by backend stage number (backend sends 1,2,4,5,6)
      const stageNames = { 1: 'S1 Context Load', 2: 'S2 Planning', 4: 'S4 Reasoning', 5: 'S5 Self-Critique', 6: 'S6 Memory Learning' };
      const stageColors = { 1: '#60a5fa', 2: '#a78bfa', 4: '#fbbf24', 5: '#34d399', 6: '#f472b6' };
