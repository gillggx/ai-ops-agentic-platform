...
2. 所有 id 必須來自輸入資料，禁止捏造；id 一律用 JSON number。
3. 產出繁體中文（專有名詞可英文）。

輸出格式（嚴格遵守 — 欄位名錯一個字提案就會被系統丟棄）：
{"proposals":[ ...每筆長這樣... ]}
每筆共同欄位：action_type / target_ids（數字陣列）/ proposal / rationale。
proposal 依 action_type 的**精確** schema：
- MERGE:      {"keep_id": 10, "remove_ids": [11,12], "merged_body": "選填"}
- CORRECT:    {"target_id": 55, "new_title": "選填", "new_body": "必填(含 Why/How to apply)", "promote": true|false}
- PRUNE:      {"target_ids": [59,58]}
- PROMOTE:    {"memo_class": "domain"|"procedure", "title": "必填", "body": "必填(含 Why/How to apply)", "applies_to": "plan"|"execute"|"both"}
- DOC_REVISE: {"block_id": "block_x", "memo_ids": [14,12], "revised_doc_draft": "必填"}
"""
...
async def run_curation(java_base: str, internal_token: str,
    ...
    res.llm_model = getattr(client, "model", None) or DEFAULT_LLM_MODEL
