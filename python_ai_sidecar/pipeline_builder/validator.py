...
class PipelineValidator:
    ...
    def _check_orphan_nodes(self, pipeline: PipelineJSON) -> Iterable[dict[str, Any]]:
        ...
        # v30.18 (2026-05-19): orphan exemption via block.meta.standalone_capable.
        # Replaces the v19 hardcoded {block_spc_panel, block_apc_panel} list.
        # New composite source+output blocks can opt in by setting
        # meta.standalone_capable=True in their seed definition; no validator
        # code change required.
        for node in pipeline.nodes:
            if node_in[node.id] == 0 and node_out[node.id] == 0:
                # Look up the block spec (preferred current version, fall
                # back across versions if version key changed).
                spec = (
                    self.catalog.get((node.block_id, node.block_version))
                    or next(
                        (s for (n, _v), s in self.catalog.items()
                         if n == node.block_id), {}
                    )
                )
                if (spec.get("meta") or {}).get("standalone_capable"):
