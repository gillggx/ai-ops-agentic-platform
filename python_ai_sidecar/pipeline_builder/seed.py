    ...
    return [
        {
            ...
            "description": (
                ...
                "**SPC 是 list 不是欄位**：\n"
                "  spc_charts = [{name, value, ucl, lcl, is_ooc, status}, ...]   一筆 process 多張圖\n"
                "  spc_summary = {ooc_count, total_charts, ooc_chart_names}\n"
                "  ⚠ 沒有 spc_xbar_value / spc_xbar_chart_value 這種扁平欄位（除非 nested=false）\n"
                "  ⚠ 想對單一 chart（xbar / r / s）做趨勢圖、回歸、SPC rule：\n"
                "     必須先接 block_unnest(column='spc_charts')\n"
                "     再接 block_filter(column='name', value='xbar')\n"
                "     展開後欄位：eventTime, toolID, step, name, value, ucl, lcl, is_ooc, status\n"
