    ...
    return [
        {
            ...
            "column_docs": [
                {
                    "col": "eventTime",
                    "type": "string (ISO 8601, camelCase — 注意是 'eventTime' 不是 snake_case 的 'event_time')",
                    "what": "此 process event 完成的精確時刻",
                },
                {
                    "col": "toolID",
                    "type": "string",
                ...
                {
                    "col": "spc_status",
                    "type": "enum<'PASS' | 'OOC'> — string, 不是 boolean",
                    "what": "此 event 整體 SPC 旗標：任一 chart 超管制 = 'OOC'；全部正常 = 'PASS'。event-level rollup, chart-level 細節見 spc_charts",
                },
                {
                    "col": "fdc_classification",
                    "type": "string | null",
                    "what": "FDC (Fault Detection & Classification) 對此 event 的分類；null = 無 FDC 警示，常見值 'FAULT' / 'WARNING'",
                },
                {
                    "col": "spc_charts",
                    "type": (
                        "list[dict] x ~12 charts per event. **每張 chart dict** 包含: "
                        "`name`: str (chart 名稱 e.g. 'xbar_chart', 'r_chart', 's_chart'); "
                        "`value`: float (此 chart 該 event 的測量值); "
                        "`ucl`: float (上管制限); `lcl`: float (下管制限); "
                        "`is_ooc`: bool (此 chart 該 event 是否超出管制限); "
                        "`status`: enum<'PASS' | 'OOC'> (per-chart 結果, 等價於 is_ooc 的字串形式)"
                    ),
                    "what": "此 event 每張 SPC chart 的完整明細測量 (source of truth — event-level 的 spc_status 由此衍生)",
                },
            ],
            "param_schema": {
                "type": "object",
        ...
        # of routinely making "sort+limit=1 kills multi-SPC" mistakes.
        {
            ...
            "param_schema": {
                ...
                "properties": {
                    "step": {
                        ...
                        "title": "Process step (必填) — 單一 'STEP_001' 或 list ['STEP_001','STEP_002']",
                    },
                    "chart_name": {
                        "anyOf": [{"type": "string"}, {"type": "array", "items": {"type": "string"}}],
                        "title": "SPC chart 名稱 (必填) — 單一 'xbar_chart' 或 list ['xbar_chart','r_chart']",
                    },
            ...
            "examples": [
                {"label": "1-block 看 EQP-01 STEP_001 xbar 過去 7 天 (典型用法)",
                 "params": {"step": "STEP_001", "chart_name": "xbar_chart",
                            "tool_id": "EQP-01", "time_range": "7d", "event_filter": "all"}},
                {"label": "機台最後一次 OOC 時的 xbar chart",
                 "params": {"step": "STEP_001", "chart_name": "xbar_chart",
                            "tool_id": "EQP-01", "event_filter": "latest_ooc",
        ...
        },
        {
            ...
            "param_schema": {
                ...
                "properties": {
                    "step": {
                        "anyOf": [{"type": "string"}, {"type": "array", "items": {"type": "string"}}],
                        "title": "Process step (必填) — 單一 'STEP_001' 或 list",
                    },
                    "chart_name": {
                        "anyOf": [{"type": "string"}, {"type": "array", "items": {"type": "string"}}],
                        "title": "APC 參數名 (必填) — 單一 'temperature' 或 list",
                    },
                    "tool_id": {"type": "string", "title": "機台 ID (留空=跨機台)"},
                    "lot_id": {"type": "string", "title": "批次 ID"},
                    "time_range": {"type": "string", "default": "7d", "title": "撈多久"},
            ...
            "examples": [
                {"label": "1-block 看 EQP-01 STEP_001 temperature 過去 7 天 (典型用法)",
