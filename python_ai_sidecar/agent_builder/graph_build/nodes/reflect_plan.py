...
== ⚠ 三條硬規範 (v24, 與 macro_plan / compile_chunk 對齊) ==

(R1) **跨 branch 取 runtime 值 → 一律用 block_join**
  當 step N 需要「另一 branch 的 runtime 輸出值」(e.g.「last X 的時間」、「max value」、
  「該 group 的 count」)，**禁止**用 block_filter+literal value，**必須**用 block_join：
    - 左表：要 filter 的全資料 branch 的 terminal
    - 右表：產出 single-row「key 值」的 branch 的 terminal
    - join key：兩表共用的 key column（e.g. eventTime）
  ❌ 不要寫 block_filter(column='eventTime', operator='==', value='???') 然後想填 placeholder
  ✅ 用 block_join(key='eventTime', how='inner') 兩個 input ports: left/right

(R2) **Pipeline input ref 一律用 `$name` syntax**
  若參數值要 reference pipeline-level input，必須是 `"$name"` (前綴 $)，且 `name`
  必須在 pipeline.inputs declared。**禁止**：
    ❌ "{some_var}"     (Python f-string 風 — 會被當 literal string)
    ❌ "<some_var>"     (XML 風 — 會被當 literal string)
    ❌ ":some_var"      (SQL 風 — 會被當 literal string)
    ❌ "$undeclared_x"  (沒在 inputs 宣告的 $ ref — runtime 拋 UNDECLARED_INPUT_REF)
  ✅ "$tool_id" (前提是已宣告), 或寫 user instruction 字面值，或改用 block_join

(R3) **Filter / set_param 的 value 必須是「三選一」**
  block_filter / set_param 的 value 字串，必須來自下列其一：
    a. **user instruction 出現過的字面值** (e.g. user 說 "EQP-07" → 用 "EQP-07")
    b. **declared pipeline input** (用 `$name` ref)
    c. **upstream sample 真實出現過的欄位值** (看 NODE TRACE 的 sample)
  其他全部禁止 — 不要捏造、不要從訓練 prior 抓字串、不要用 placeholder template。
  若沒有合適來源 → 改用 block_join（見 R1）。
