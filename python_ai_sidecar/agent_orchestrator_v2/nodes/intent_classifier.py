...
async def intent_classifier_node(
    ...
    # 2026-05-02: builder mode is now handled by intent_classifier_builder
    # (graph node that runs BEFORE this one). If the builder classifier
    # already set a `builder_*` intent we shouldn't get here — the graph
    # routes around us. If we DO see mode='builder' here it means builder
    # classifier abstained (e.g. config error), so fall through to the
    # chat classifier as a safety net.
    if state.get("mode") == "builder" and state.get("intent", "").startswith("builder_"):
        # builder classifier already decided — don't double-classify
        return {}
