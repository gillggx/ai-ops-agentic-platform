    ...
    try:
        ...
        if get_settings().PIPELINE_ONLY_MODE and mode != "builder":
            ...
            system_text += (
                ...
                "     a. **Ambiguity check FIRST** — before any other tool call.\n"
                "        模糊條件（任一中即視為模糊 → 必須先 confirm_pipeline_intent）：\n"
                ...
                "          - 沒明確指定產出形式（Alert? Chart? Table? Multiple charts grouped?）\n"
                "        → 直接呼 `confirm_pipeline_intent(inputs, logic, presentation, alternatives)`，\n"
                "          **絕對不要先 search_published_skills 或 build_pipeline_live**\n"
                "          （user 要的是什麼還沒定，搜也搜不準，建出來通常還是錯的）。\n"
                "        判斷標準：你能用一句話寫出 logic + presentation 沒有任何「我猜...」的地方嗎？\n"
                "          能 → 算具體。不能（任何「應該是想看 X 趨勢吧」念頭）→ 先 confirm。\n"
                "        等 user 點 ✅ 後（下一條訊息會帶 `[intent_confirmed:<id>]` prefix）再 build。\n"
                "     b. Call `search_published_skills(query=<user goal>)`.\n"
                "     c. If a result matches well, call `invoke_published_skill(slug, inputs)`.\n"
                "     d. **If no good match**: 告訴 user「找不到現成 skill，要不要我幫你建一條？」，\n"
                "        等 user 「好/可以/ok」，再 `build_pipeline_live(goal=\"...\")`.\n"
                "     e. 若使用者一開始就明確表達要「建 pipeline / 建新 skill」可直接呼叫。\n"
                "\n"
