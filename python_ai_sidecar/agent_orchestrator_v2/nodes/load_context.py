    ...
    try:
        ...
        if get_settings().PIPELINE_ONLY_MODE and mode != "builder":
            ...
            system_text += (
                ...
                "        模糊（含「該機台/該站/最近/分析/確認」、跨 metric 來源、沒指定呈現方式）\n"
                "        → 直接呼 `confirm_pipeline_intent(inputs, logic, presentation)`，\n"
                "        **不要先 search_published_skills**（user 要的是什麼還沒定，搜也搜不準）。\n"
                "        等 user 點 ✅ 後再走 b/c。具體 prompt 才繼續往下。\n"
