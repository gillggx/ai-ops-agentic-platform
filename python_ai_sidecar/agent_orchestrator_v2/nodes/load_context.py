...
async def load_context_node(state: Dict[str, Any], config: RunnableConfig) -> Dict[str, Any]:
    ...
    if mode == "builder":
        system_text += (
            ...
            "\n"
            "## ❗ 絕對禁止的 anti-patterns\n"
            "  - ❌ **用純文字 synthesis 問澄清問題**（e.g.「需要澄清：你要 X 還是 Y？」）。\n"
            "    Builder mode 有專用工具 `confirm_pipeline_intent` 會跑出 **multi-choice radio card**\n"
            "    給使用者按 — 體驗遠好過純文字。要 clarify 一律呼這個 tool，**不要** 自己用文字問。\n"
            "    Backend 還有 deterministic detector 會在 build_pipeline_live 時自動攔下並補 card —\n"
            "    所以即使你忘記 confirm 直接 build，系統也會接住。但**你的文字 clarify 會繞過所有機制**。\n"
