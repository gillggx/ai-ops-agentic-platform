    ...
    try:
        ...
        if get_settings().PIPELINE_ONLY_MODE and mode != "builder":
            ...
            system_text += (
                ...
                "\n"
                "## Tool choice algorithm — STRICT ORDER, do not skip stages\n"
                "1. **Knowledge-only** question (e.g. \"WECO R5 是什麼\")\n"
                "     → Answer as plain text. No tool call.\n"
                "2. **Data / analytical** question — must walk these stages **in order**：\n"
                "\n"
                "     ### Stage A · Ambiguity gate（FIRST — 不可跳過）\n"
                "     在呼任何其他 tool 之前，先做這個檢查。模糊條件（任一中即模糊）：\n"
                "       - 含時間/位置模糊指代：最後一次 / 最近 / 上次 / 該機台 / 該站 / 這個 lot\n"
                "       - 含動詞模糊：分析 / 確認 / 看一下 / 檢查（注意：「檢查 X 趨勢」中的「趨勢」\n"
                "         其實沒指定 chart 類型 — xbar trend? r chart? 多 step 還單 step? → 模糊）\n"
                "       - 跨 metric 來源（同時提 SPC + APC + FDC 沒指定要看哪個）\n"
                "       - **沒明確指定產出形式（Alert? Chart? Table? Scalar?）— 注意這條最常被忽略**\n"
                "         例：「各站點 OOC rate」→ 模糊（table？bar chart？pareto？time-series？沒講）\n"
                "         例：「OOC 趨勢」→ 模糊（單線？多 step 多線？per-tool 分組？沒講）\n"
                "\n"
                "     模糊判斷標準：你能用一句話寫出 **inputs + logic + presentation** 三項全部具體\n"
                "     沒有任何「我猜...」的地方嗎？\n"
                "       能 → 算具體，跳到 Stage B\n"
                "       不能（任何「應該是想看 X 趨勢吧」「假設用 table 呈現」念頭）→ 先 confirm\n"
                "\n"
                "     模糊 → 直接呼 `confirm_pipeline_intent(inputs, logic, presentation, alternatives)`。\n"
                "     **絕對不要先 search_published_skills 或 build_pipeline_live** —\n"
                "     user 要的是什麼還沒定，搜也搜不準，建出來通常還是錯的。\n"
                "     等 user 點 ✅ 後（下一條訊息會帶 `[intent_confirmed:<id>]` prefix）再走 Stage B。\n"
                "\n"
                "     ### Stage B · Skill discovery（具體之後才到這）\n"
                "     a. Call `search_published_skills(query=<user goal>)`.\n"
                ...
                "\n"
                "     ⚠️ Anti-pattern（別這樣做）：\n"
                "       ❌ search_published_skills → 沒命中 → build_pipeline_live（中間沒 confirm）\n"
                "       ❌ 「OOC rate 應該是要表格吧」就直接 build → 用戶想要 chart 就白做了\n"
                "       ✅ 任一模糊條件成立 → confirm_pipeline_intent 先停下來問\n"
                "\n"
