...
- 直接自然講話。可以閒聊、可以解釋、可以回答「你能幫我做什麼」——用人話講清楚，
  **絕對不要**丟制式選單卡逼使用者選。
- 判斷清楚再用工具：要「建新圖」用 build_pipeline；要「改現在這張」用
  modify_current_chart（畫面上沒圖就先建）；只是查 / 問則用查詢工具或直接回答。
- build_pipeline 會花幾十秒，呼叫前可以先簡短回一句「幫你建，稍等」。
...
_TOOLS: List[Dict[str, Any]] = [
    ...
    {
        "name": "build_pipeline",
        "description": "使用者要建一張新的圖 / pipeline（例：畫 EQP-01 xbar SPC 趨勢圖、"
                       "統計各機台 OOC 次數）時用。輸入一句自然語言需求，我會用現有的建置引擎"
                       "把圖建出來並顯示。建好前會花幾十秒。",
        "input_schema": {"type": "object", "properties": {
...
async def _run_tool(name: str, inp: Dict[str, Any], ctx: Dict[str, Any]) -> AsyncIterator[tuple]:
    ...
    try:
        ...
        # ── build_pipeline — wraps the UNCHANGED Planner & Builder graph ──
        if name == "build_pipeline":
            ...
            yield ("event", {"type": "stage_update", "stage": 1, "label": "規劃 + 建置中…", "status": "running"})
            final = None
            async for ev in stream_graph_build(instruction=instr, skip_confirm=True,
                                               user_id=ctx["user_id"], session_id=ctx["session_id"]):
                if ev.type == "phase_completed":
                    yield ("event", {"type": "stage_update", "stage": 2,
                                     "label": str((ev.data or {}).get("rationale") or "建置中"), "status": "running"})
                elif ev.type == "done":
                    final = (ev.data or {}).get("pipeline_json") or final
            if not final or not (final.get("nodes")):
                yield ("result", {"status": "failed",
                                  "message": "這次沒建出可用的圖；需求可能要更具體（機台 / 站點 / 要看什麼）。"})
                return
            try:
