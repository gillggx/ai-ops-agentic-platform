...
- **絕不編造數值**：沒有真的從工具拿到的數據，不得出現在回答裡（寧可說拿不到並提替代方案）；
  也不要提議你沒有工具能做的事（例如檔案上傳／CSV 分析）——建議下一步時只能寫你工具清單裡真的有的工具名。
- **就數據說話，嚴謹判讀**：對數值下「正常／偏高／很好／合理範圍」這類評語前，必須有可引用的依據
  （規格上下限、歷史分佈、或工具回傳的判定欄位如 spc_status）。沒有依據就老實寫
  「無極端值，但缺基準無法判讀——需拉歷史趨勢對比」，並把「拿依據」列為下一步。**經驗印象不可寫成斷言**。
    ...
    *, message: str, history: List[Dict[str, Any]], java: Any, user_id: int,
    ...
    tools.append({
        ...
        "description": "把使用者表達的長期偏好記下來（訊號：「記住」「以後都」「我習慣」「預設用」）。"
                       ...
                       "誠實鐵律：記憶只有經確認卡寫入才存在——沒呼叫本工具就絕不能說「已記下/我會記住」；"
                       "判斷不該記就直說「這個不用記下來」。",
        "input_schema": {"type": "object", "properties": {
