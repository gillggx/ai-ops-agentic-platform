...
自動產生於 2026-07-13，來源 `python_ai_sidecar/pipeline_builder/seed.py`（block 文件的單一來源 — GUI 表單、agent 目錄與本檔皆出自它）。
**不要手改本檔** — 改 seed 後跑 `python3 tools/blocks_doc/generate.py` 重生。

共 58 個積木。行為驗證：`python_ai_sidecar/tests/test_blocks_core.py`。

## 目錄

- **check**：[`block_step_check`](#block-step-check)
- **邏輯/統計 Logic & Stats**：[`block_any_trigger`](#block-any-trigger)、[`block_consecutive_rule`](#block-consecutive-rule)、[`block_correlation`](#block-correlation)、[`block_cpk`](#block-cpk)、[`block_hypothesis_test`](#block-hypothesis-test)、[`block_linear_regression`](#block-linear-regression)、[`block_threshold`](#block-threshold)、[`block_weco_rules`](#block-weco-rules)
- **輸出 Output（圖表/表格/告警）**：[`block_alert`](#block-alert)、[`block_apc_panel`](#block-apc-panel)、[`block_bar_chart`](#block-bar-chart)、[`block_box_plot`](#block-box-plot)、[`block_chart`](#block-chart)、[`block_data_view`](#block-data-view)、[`block_defect_stack`](#block-defect-stack)、[`block_ewma_cusum`](#block-ewma-cusum)、[`block_heatmap_dendro`](#block-heatmap-dendro)、[`block_histogram_chart`](#block-histogram-chart)、[`block_imr`](#block-imr)、[`block_line_chart`](#block-line-chart)、[`block_parallel_coords`](#block-parallel-coords)、[`block_pareto`](#block-pareto)、[`block_probability_plot`](#block-probability-plot)、[`block_scatter_chart`](#block-scatter-chart)、[`block_spatial_pareto`](#block-spatial-pareto)、[`block_spc_panel`](#block-spc-panel)、[`block_splom`](#block-splom)、[`block_trend_wafer_maps`](#block-trend-wafer-maps)、[`block_variability_gauge`](#block-variability-gauge)、[`block_wafer_heatmap`](#block-wafer-heatmap)、[`block_xbar_r`](#block-xbar-r)
- **資料源 Source**：[`block_list_objects`](#block-list-objects)、[`block_mcp_call`](#block-mcp-call)、[`block_process_history`](#block-process-history)、[`block_rework_request`](#block-rework-request)
- **處理 Transform**：[`block_apc_long_form`](#block-apc-long-form)、[`block_compute`](#block-compute)、[`block_count_rows`](#block-count-rows)、[`block_delta`](#block-delta)、[`block_ewma`](#block-ewma)、[`block_filter`](#block-filter)、[`block_find`](#block-find)、[`block_groupby_agg`](#block-groupby-agg)、[`block_histogram`](#block-histogram)、[`block_join`](#block-join)、[`block_mcp_foreach`](#block-mcp-foreach)、[`block_pluck`](#block-pluck)、[`block_rolling_window`](#block-rolling-window)、[`block_select`](#block-select)、[`block_shift_lag`](#block-shift-lag)、[`block_sort`](#block-sort)、[`block_spc_long_form`](#block-spc-long-form)、[`block_streak`](#block-streak)、[`block_time_bucket`](#block-time-bucket)、[`block_union`](#block-union)、[`block_unnest`](#block-unnest)、[`block_unpivot`](#block-unpivot)

# check

## block_step_check

- **狀態**：production　**版本**：1.0.0
- **輸入**：`data` (dataframe)
- **輸出**：`check` (dataframe)
...
| 參數 | 型別 | 必填 | 說明 |
|---|---|---|---|
| `aggregate` | string |  | 可選：count, sum, mean, max, min, last, exists |
| `column` | string |  |  |
| `operator` | string | 是 | 可選：>=, >, =, ==, <, <=, changed, drift |
| `threshold` | number |  |  |
| `baseline` | any |  |  |
| `condition_label` | string |  |  |
| `severity` | string |  | 可選：LOW, MEDIUM, HIGH |
...
- **狀態**：production　**版本**：1.0.0
- **輸入**：`trigger_1` (bool), `trigger_2` (bool), `trigger_3` (bool), `trigger_4` (bool), `evidence_1` (dataframe), `evidence_2` (dataframe), `evidence_3` (dataframe), `evidence_4` (dataframe)
- **輸出**：`triggered` (bool), `evidence` (dataframe)
...
| 參數 | 型別 | 必填 | 說明 |
|---|---|---|---|
| `flag_column` | string | 是 |  |
| `count` | integer | 是 | 連續筆數 (常見 3-8) |
| `sort_by` | string | 是 | Sort by (必填) |
| `group_by` | string |  | Group by (選填) |
...
| 參數 | 型別 | 必填 | 說明 |
|---|---|---|---|
| `columns` | array |  | 納入欄位（省略=全部數值欄） |
| `method` | string |  | 可選：pearson, spearman, kendall |
| `target` | string |  | 排行模式：每欄 vs 這個欄位的相關排行 |
...
| `usl` | number |  | USL (upper spec limit) |
| `lsl` | number |  | LSL (lower spec limit) |
| `group_by` | string |  | Group by (選填) |
...
| 參數 | 型別 | 必填 | 說明 |
|---|---|---|---|
| `test_type` | string | 是 | 可選：t_test, anova, chi_square |
| `value_column` | string |  | 數值欄位 (t_test/anova) |
| `group_column` | string | 是 |  |
| `target_column` | string |  | 類別欄位 (chi_square) |
| `alpha` | number |  |  |
...
- **狀態**：production　**版本**：1.0.0
- **輸入**：`data` (dataframe)
- **輸出**：`stats` (dataframe), `data` (dataframe), `ci` (dataframe)
...
|---|---|---|---|
| `column` | string | 是 |  |
| `bound_type` | string |  | Mode A: UCL/LCL 模式 可選：upper, lower, both |
| `upper_bound` | number |  |  |
| `lower_bound` | number |  |  |
| `operator` | string |  | Mode B: 通用比較運算子 可選：==, !=, >=, <=, >, < |
| `target` | any |  | Mode B: 目標值（數字或字串） |
...
| 參數 | 型別 | 必填 | 說明 |
|---|---|---|---|
| `value_column` | string | 是 | 監控指標欄位 |
| `center_column` | string |  | Center Line 欄位（選填） |
| `sigma_source` | string |  | σ 來源 可選：from_ucl_lcl, from_value, manual |
| `ucl_column` | string |  | UCL 欄位（sigma_source=from_ucl_lcl 時用） |
| `manual_sigma` | number |  | σ 數字（sigma_source=manual 時用；必為正數） |
| `rules` | array |  | 啟用規則 (Nelson R1..R8；預設 4 條常用) |
| `group_by` | string |  | Group by（選填，每組獨立評估） |
| `sort_by` | string |  | Sort by（選填，預設 eventTime） |
...
- **狀態**：production　**版本**：1.0.0
- **輸入**：`triggered` (bool), `evidence` (dataframe)
- **輸出**：`alert` (dataframe)
...
| 參數 | 型別 | 必填 | 說明 |
|---|---|---|---|
| `severity` | string | 是 | 可選：LOW, MEDIUM, HIGH, CRITICAL |
...
| 參數 | 型別 | 必填 | 說明 |
|---|---|---|---|
| `step` | any | 是 | Process step (必填) — 單一 'STEP_001' 或 list |
| `chart_name` | any | 是 | APC 參數名 (必填) — 單一 'temperature' 或 list |
| `tool_id` | string |  | 機台 ID (留空=跨機台) |
| `lot_id` | string |  | 批次 ID |
| `time_range` | string |  | 撈多久 |
| `limit` | integer |  | 撈幾筆 |
| `event_filter` | string |  | 事件範圍 可選：latest_drift, latest_event, all, custom_time |
| `event_time` | string |  | 指定 timestamp (ISO) |
| `show_only_violations` | boolean |  | 只秀 drift 的參數 |
| `color_by` | string |  | 依此欄分色 |
| `value_color` | string |  | Value 線顏色 |
| `bound_color` | string |  | Bound 線顏色 |
| `title` | string |  | 圖表標題 |
...
| 參數 | 型別 | 必填 | 說明 |
|---|---|---|---|
| `chart_type` | string | 是 | 可選：line, bar, scatter, area, boxplot, heatmap, distribution, table |
| `x` | string |  |  |
| `y` | ['string', 'array'] |  | y (string or array) |
| `y_secondary` | ['string', 'array'] |  | 右側 Y 軸欄位 (選填，string/array) |
| `value_column` | string |  | heatmap/distribution value 欄位 |
| `group_by` | string |  | boxplot group_by (boxplot 專用) |
| `bins` | integer |  | 直方圖 bins (distribution) |
| `usl` | number |  | USL (distribution / SPC) |
| `lsl` | number |  | LSL (distribution / SPC) |
| `show_sigma_lines` | array |  | Distribution σ 線 (1..6) |
| `sigma_zones` | array |  | SPC ±σ zones 線 (e.g. [1, 2]) |
| `color` | string |  | color (series within ONE chart, same y-axis) |
| `facet` | string |  | facet (split into N independent charts, one per group) |
| `title` | string |  | 標題 |
| `color_scheme` | string |  | Color scheme 可選：, tableau10, set2, blues, reds, greens |
| `show_legend` | boolean |  | 顯示圖例 |
| `width` | integer |  |  |
| `height` | integer |  |  |
| `ucl_column` | string |  | UCL 欄位 (SPC) |
| `lcl_column` | string |  | LCL 欄位 (SPC) |
| `center_column` | string |  | Center 欄位 (SPC，選填) |
| `highlight_column` | string |  | OOC highlight 欄位 (bool) |
| `sequence` | integer |  | 顯示順序（流水號） |
...
| 參數 | 型別 | 必填 | 說明 |
|---|---|---|---|
| `title` | string |  | 標題（預設 'Data View'） |
| `description` | string |  | 副標（選填） |
| `columns` | array |  | 要顯示的欄位（未指定則全部） |
| `highlight_rules` | array |  | 條件格式化 — 命中的 cell 套色 |
| `max_rows` | integer |  | 最多顯示列數（預設 200） |
| `sequence` | integer |  | 多視圖時的排序（ascending） |
...
| `x_column` | string |  |  |
| `y_column` | string |  |  |
| `defect_column` | string |  |  |
| `codes` | array |  |  |
| `wafer_radius_mm` | number |  | Wafer 半徑 (mm) |
| `notch` | string |  | 可選：top, bottom, left, right |
| `title` | string |  |  |
...
| `values` | array |  |  |
| `value_column` | string |  |  |
| `mode` | string |  | 可選：ewma, cusum |
| `lambda` | number |  |  |
| `k` | number |  | CUSUM k (reference value, typical 0.3-0.7σ) |
| `h` | number |  | CUSUM h (decision interval, typical 3-5σ) |
| `target` | ['number', 'null'] |  |  |
| `title` | string |  |  |
...
| 參數 | 型別 | 必填 | 說明 |
|---|---|---|---|
| `x` | string |  | x 欄位；省略或填 'sequence' = 照資料順序 1..N（不需要時間欄） |
| `y` | string / array | 是 |  |
| `y_secondary` | array |  |  |
| `series_field` | string |  |  |
| `rules` | array |  |  |
...
| `facet` | string |  | facet — split into N panels by column |
| `title` | string |  |  |
| `style` | object |  |  |
| `tooltip_fields` | array |  |  |
| `weco_annotate` | boolean |  |  |
...
| 參數 | 型別 | 必填 | 說明 |
|---|---|---|---|
| `step` | any | 是 | Process step (必填) — 單一 'STEP_001' 或 list ['STEP_001','STEP_002'] |
| `chart_name` | any | 是 | SPC chart 名稱 (必填) — 單一 'xbar_chart' 或 list ['xbar_chart','r_chart'] |
| `tool_id` | string |  | 機台 ID (留空=跨機台) |
| `lot_id` | string |  | 批次 ID |
| `time_range` | string |  | 撈多久 |
| `limit` | integer |  | 撈幾筆 |
| `event_filter` | string |  | 事件範圍 可選：latest_ooc, latest_event, all, custom_time |
| `event_time` | string |  | 指定 timestamp (ISO) |
| `show_only_violations` | boolean |  | 只秀 OOC 的 charts |
| `color_by` | string |  | 依此欄分色 (e.g. toolID / lotID) |
| `value_color` | string |  | Value 線顏色 |
| `bound_color` | string |  | UCL/LCL 線顏色 |
| `title` | string |  | 圖表標題 |
| `style` | object |  | 圖表外觀 — σ 區帶/軸標籤（透傳到每格子圖） |
| `tooltip_fields` | array |  | 滑鼠提示額外欄位 (e.g. ['lotID'])，透傳到每格 |
| `weco_annotate` | boolean |  | 違規點提示顯示違規說明 |
...
| `pm_column` | string |  |  |
| `wafer_radius_mm` | number |  | Wafer 半徑 (mm) |
| `cols` | integer |  | 每列 wafer 數 (chart layout) |
| `grid_n` | integer |  |  |
| `notch` | string |  | 可選：top, bottom, left, right |
| `title` | string |  |  |
...
| `notch` | string |  | 可選：top, bottom, left, right |
| `unit` | string |  |  |
| `color_mode` | string |  | 可選：viridis, diverging |
| `show_points` | boolean |  |  |
| `grid_n` | integer |  |  |
| `title` | string |  |  |
...
| 參數 | 型別 | 必填 | 說明 |
|---|---|---|---|
| `kind` | string | 是 | 物件類別 可選：tool, lot, step, apc, spc |
| `args` | object |  | MCP 參數 (object) |
...
| 參數 | 型別 | 必填 | 說明 |
|---|---|---|---|
| `mcp_name` | string | 是 | MCP 名稱 |
| `args` | object |  | 呼叫參數 (object) |
...
| 參數 | 型別 | 必填 | 說明 |
|---|---|---|---|
| `tool_id` | string |  | 機台 ID (三擇一，單值) |
| `lot_id` | string |  | 批次 ID (三擇一) |
| `step` | string |  | 站點 Step (三擇一) |
| `object_name` | string |  | 資料維度 (選填，留空=全部) 可選：, SPC, APC, DC, RECIPE, FDC, EC |
| `time_range` | string |  | 時間窗 (Nh / Nd，e.g. 1h, 24h, 48h, 72h, 7d, 30d) |
| `event_time` | string |  | 精確時間 (ISO8601，選填) |
| `limit` | integer |  | 筆數上限 |
| `nested` | boolean |  | 回傳 hierarchical shape（保留 SPC/APC/DC/RECIPE/FDC/EC nested + 預算 spc_summary） |
...
| 參數 | 型別 | 必填 | 說明 |
|---|---|---|---|
| `lot_id` | string | 是 | Lot ID (必填) |
| `step` | string |  | Step (選填) |
| `flow_id` | string |  | Flow ID (選填) |
...
| 參數 | 型別 | 必填 | 說明 |
|---|---|---|---|
| `column` | string | 是 | 新欄位名稱 |
| `expression` | object | 是 | Expression tree |
...
|---|---|---|---|
| `column` | string | 是 |  |
| `operator` | string | 是 | 可選：==, =, !=, >, <, >=, <=, contains, in |
| `value` | any |  |  |
| `order_by` | string |  |  |
| `order_dir` | string |  | 可選：asc, desc |
| `take` | any |  |  |
...
aggregations (array, opt) **多重聚合一顆搞定**：[{column, func, as?}, ...]，例 [{column:'value',func:'mean'},{column:'value',func:'count'}] → 輸出 value_mean + value_count。給了就忽略下面的 column/aggregate
           ✅ 單欄 string:  'toolID'
...
| 參數 | 型別 | 必填 | 說明 |
|---|---|---|---|
| `aggregations` | array |  | 多重聚合 [{column, func, as?}]（給了就忽略 column/aggregate） |
| `group_by` | string / array | 是 | Group by column(s) — string for single, list for multi |
| `column` | string |  | Aggregation target column |
| `aggregate` | string |  | Aggregation function 可選：mean, sum, count, min, max, median, std |
| `agg_column` | string |  | [legacy alias] use `column` instead |
| `agg_func` | string |  | [legacy alias] use `aggregate` instead 可選：mean, sum, count, min, max, median, std |

## block_histogram

- **狀態**：production　**版本**：1.0.0
- **輸入**：`data` (dataframe)
- **輸出**：`data` (dataframe), `stats` (dataframe)
...
- **狀態**：production　**版本**：1.0.0
- **輸入**：`left` (dataframe), `right` (dataframe)
- **輸出**：`data` (dataframe)
...
| 參數 | 型別 | 必填 | 說明 |
|---|---|---|---|
| `key` | string | 是 | Join key(s); 逗號分隔多欄 |
| `how` | string |  | 可選：inner, left, right, outer |
...
|---|---|---|---|
| `mcp_name` | string | 是 |  |
| `args_template` | object | 是 | MCP args (可用 $col 引用 row 欄位) |
| `result_prefix` | string |  | 結果欄位前綴 |
...
| 參數 | 型別 | 必填 | 說明 |
|---|---|---|---|
| `column` | string | 是 | 目標欄位 |
| `window` | integer | 是 | Window size (常見 3-30) |
| `func` | string | 是 | 可選：mean, std, min, max, sum, median |
| `min_periods` | integer |  | min_periods |
| `group_by` | string |  | Group by（選填） |
| `sort_by` | string |  | Sort by（選填，預設 eventTime） |
...
|---|---|---|---|
| `column` | string | 是 | 目標欄位 |
| `offset` | integer | 是 | Offset（正=過去、負=未來；常見 1-7） |
| `group_by` | string |  | Group by（選填；各組內獨立 shift） |
| `sort_by` | string |  | Sort by（選填，預設 eventTime） |
| `compute_delta` | boolean |  | 同時輸出 delta 欄位 |
...
| 參數 | 型別 | 必填 | 說明 |
|---|---|---|---|
| `columns` | array | 是 | 排序欄位 (字串清單，或 {column, order}) |
| `limit` | integer |  | Top-N (選填) |
...
```
⚠ DEPRECATED (2026-05-13)。block_process_history 預設改為 nested=true，下游 想要 long-form 直接接 block_unnest(column='spc_charts') 就好，shape 完全一樣。保留是為了舊 pipeline 仍可載入；新建議全部改用 unnest。
...
== When to use ==
- [ok] 「過去 N 天 OOC 次數 by hour」→ time_bucket(eventTime,1h) → groupby_agg(group_by=time_bucket, count) → bar_chart
...
|---|---|---|---|
| `column` | string | 是 |  |
| `interval` | string |  | 可選：15min, 30min, 1h, 4h, 1d |
| `output_column` | string |  |  |
| `tz` | string |  |  |
| `label` | string |  | 可選：start, center |

## block_union

- **狀態**：production　**版本**：1.0.0
- **輸入**：`primary` (dataframe), `secondary` (dataframe)
- **輸出**：`data` (dataframe)
...
| 參數 | 型別 | 必填 | 說明 |
|---|---|---|---|
| `on_schema_mismatch` | string |  | 欄位不符時 可選：outer, intersect |
...
| 參數 | 型別 | 必填 | 說明 |
|---|---|---|---|
| `id_columns` | array | 是 | 保留欄位 |
| `value_columns` | array | 是 | 要 melt 的欄位 |
| `variable_name` | string |  | 分類欄名 |
| `value_name` | string |  | 值欄名 |
