...
E1–E4 依建議定案。Commits `d126e2e2` + source-CHECK fix;V70 已套 prod。

| # | 條款 | 實測 |
|---|---|---|
| D1 | flag OFF 零寫入 | OFF 跑編輯 build → 兩表零新列(4=4) |
| D2 | legacy 列不變 | 26 列 memo_class IS NULL,檢索行為不變 |
| **D3** | **學習閉環** | **關閉**:build A 編輯 → 筆記 41/42 落庫、embedding 30s 自動補;build B 檢索 **uses=1**(筆記 42 進 planner hint) |
| D4 | 分類正確 | 12小時→preference、bar chart→presentation |
| D5 | reject→doc memo | sink e2e 過(id=1 + dedup);**自然觸發待觀察**:兩輪 probe 被拒 phase 全走 repair/handover(拒後通過今日未發生),glue 與 gate 已證的 param_reject_fix 同路 |
| D6 | repair→correction | **實戰觸發**:spc-cpk p4 掙扎 → 筆記 43(retry)/44(handover),applies_to=execute |
| D7 | dedup | 重放同編輯 → agent_fast 仍 2 列 |
| D8 | caps | 單測覆蓋(3/5 上限、可 env 調) |
| D9 | report 有 doc-memo section | pending_doc_memos 出數字 |
| D10 | SLASH-17 零回歸 | **✅ 過**(key 補額後重跑):16/17 finished,strict **14 MATCH+1 OVER**,cache **55.3%**(帶內);唯一 FAIL=ooc-ranking provider timeout(status=None),非 code |

**污染事件與修正(2026-07-03,重要)**:灰度中觀察到 W3 寫入的 live correction
被 execute-layer 檢索召回(uses 到 5),把 3 個原本會過的案子帶偏成 handover。
因果實驗:停用全部 agent_fast correction → 那 3 案 **0/3 → 3/3**。修正(commit
後續):**W3 只在自癒成功(retry)寫、correction 一律 active=false 草稿**(待
Supervisor 晉升);preference/presentation(user 明改)維持即時生效——D3 閉環那條健康。
重跑最終閘門確認修正後 correction 全 draft(12 筆 active=f)、D5 自然 memo 4 筆
(block_select/weco_rules/spc_panel,拒後通過)、cache 55.3%、品質零回歸。

**設計啟示**:episodic→檢索的自我強化迴路夠強,所以防護(可停用 / draft /
Supervisor curation)是必要而非裝飾——本次污染事件正是它們存在的實證理由。

**已修 bug**:agent_knowledge source CHECK 不含 agent_fast(e2e 揪出,已修)。
