...
| D10 | SLASH-17 零回歸 | **部分**:前 10 案 10/10 OK(xbar_r 正確)後 **OpenRouter key 額度耗盡**(403 Key limit exceeded)→ case 11-17 瞬敗。非 code 回歸;key 補額後重跑全套 |

**已知待辦**:D5 自然案例(canary 觀察)、D10 全套重跑(blocked on key)、
e2e 揪出並修復的 bug:agent_knowledge source CHECK 不含 agent_fast(已修)。
