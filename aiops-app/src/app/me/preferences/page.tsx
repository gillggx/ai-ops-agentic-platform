...
export default function MyPreferencesPage() {
  const t = useTranslations("me.prefs");
  const APPLIES_LABEL: Record<string, string> = {
    plan: t("applyPlan"), execute: t("applyExecute"), both: t("applyBoth"),
  };
  ...
  const save = async (p: Partial<Pref>, id?: number) => {
    ...
    } catch (e) { setMsg(e instanceof Error ? e.message : t("saveFail")); }
  };
  ...
  return (
    <div style={{ maxWidth: 760, margin: "0 auto", padding: "28px 20px 60px",
      ...
      {loading && <div style={{ color: "#8b90a7", fontSize: 13 }}>{t("loading")}</div>}
            ...
            <>
              ...
              <div style={{ display: "flex", alignItems: "center", marginTop: 9 }}>
                <span style={{ fontSize: 10.5, color: "#a0a4b5", fontFamily: "ui-monospace, monospace" }}>
                  #{p.id} ・ {t("usedTimes", { n: p.uses ?? 0 })}
                </span>
                <span style={{ flex: 1 }} />
                <button onClick={() => setEditing(p)} style={btn(false)}>{t("edit")}</button>
                <button onClick={() => void del(p.id)} style={{ ...btn(false), color: "#B91C1C", marginLeft: 6 }}>{t("delete")}</button>
              </div>
            </>
          )}
        </div>
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
      ...
      <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
        ...
        <span style={{ flex: 1 }} />
        <button onClick={onCancel} style={btn(false)}>{t("cancel")}</button>
        <button onClick={() => title.trim() && body.trim()
          && onSave({ title: title.trim(), body: body.trim(), applies_to: appliesTo })}
          style={{ ...btn(true), marginLeft: 6 }}>{t("save")}</button>
      </div>
    </div>
  );
}
