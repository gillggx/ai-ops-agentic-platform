...
from app.ontology.models.system_parameter import SystemParameter as SystemParameterModel
from app.ontology.models.agent_session import AgentPreference as UserPreferenceModel
...
{_OUTPUT_ROUTING}
    ...
    async def _load_soul(self, user_id: int) -> str:
        """Load Soul prompt with strict priority:
          1. user_preferences.soul_override  (per-user admin override — highest priority)
          2. system_parameters[AGENT_SOUL_PROMPT]  (global DB override — editable in UI)
          3. _DEFAULT_SOUL  (hardcoded fallback — used on fresh DB, never shown to end users)
        """
        # Priority 1: per-user soul override (set by admin via user preference UI)
        ...
        # Priority 2: global soul from system_parameters (editable without deploy)
        result = await self._db.execute(
        ...
        # Priority 3: hardcoded fallback (only on fresh/empty DB)
