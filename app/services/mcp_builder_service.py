...
class MCPBuilderService:
    ...
    Prompt loading priority for all 4 LLM system prompts:
      1. Caller passes explicit override (prompt_template / system_prompt parameter)
      2. system_parameters table (keyed by SystemParameter.KEY_PROMPT_*)
      3. Module-level _DEFAULT_* constant (hardcoded fallback, always present)
    """

    # Well-known SystemParameter keys for prompt overrides
    KEY_PROMPT_MCP_GENERATE = "PROMPT_MCP_GENERATE"
    KEY_PROMPT_MCP_TRY_RUN = "PROMPT_MCP_TRY_RUN"
    KEY_PROMPT_SKILL_DIAGNOSIS = "PROMPT_SKILL_DIAGNOSIS"
    KEY_PROMPT_SKILL_DIAG_CODE = "PROMPT_SKILL_DIAG_CODE"

    def __init__(self, sp_repo=None, **_kwargs) -> None:
        """
        Args:
            sp_repo: Optional SystemParameterRepository — when provided, prompts are
                     looked up in system_parameters before falling back to hardcoded defaults.
                     The **_kwargs absorbs legacy positional repos (mcp_repo, ds_repo) that
                     callers may pass but this service no longer needs.
        """
        self._llm = get_llm_client()
        self._sp_repo = sp_repo  # may be None (stateless callers)

    async def _load_prompt(self, key: str, fallback: str) -> str:
        """Return DB-stored prompt for *key* if present, otherwise return *fallback*."""
        if self._sp_repo is None:
            return fallback
        try:
            sp = await self._sp_repo.get_by_key(key)
            if sp and sp.value:
                return sp.value
        except Exception:
            pass
    ...
    async def generate_all(
        ...
        """
        template = prompt_template or await self._load_prompt(
            self.KEY_PROMPT_MCP_GENERATE, _DEFAULT_GENERATE_PROMPT
        )
    ...
    async def generate_for_try_run(
        ...
        """
        sys_prompt = system_prompt or await self._load_prompt(
            self.KEY_PROMPT_MCP_TRY_RUN, _DEFAULT_TRY_RUN_SYSTEM_PROMPT
        )
...
{{
  ...
  "fix_suggestion": "建議使用者採取的行動（1-2句）"
    ...
    async def try_diagnosis(
        ...
        """
        sys_prompt = system_prompt or await self._load_prompt(
            self.KEY_PROMPT_SKILL_DIAGNOSIS, _DEFAULT_DIAGNOSIS_SYSTEM_PROMPT
        )
...
- ⚠️ **Exception Handling 鐵律**：主診斷邏輯必須包在 try/except 中，格式如下：
  ...
  exception message 必須使用 `f"診斷執行異常：{{e}}"` 格式，不可自訂其他格式。"""
        ...
        # Load system prompt from DB (falls back to hardcoded default)
        _skill_sys_prompt = await self._load_prompt(
            self.KEY_PROMPT_SKILL_DIAG_CODE, _DEFAULT_SKILL_DIAG_SYSTEM_PROMPT
        )
